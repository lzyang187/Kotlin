...
/**
 * 1、一个线程中可以运行数以百万的协程
 * 2、协程不会绑定到某一特定线程，例如协程可以在主线程启动、暂停，在后台线程恢复
 * 3、挂起函数：在被挂起点以非阻塞方式暂停代码的执行。当前正在执行挂起函数的协程与其正在运行的线程分离，然后一直处于等待状态
 * 直到该挂起函数恢复执行（可能在另一线程上），此期间线程可以自由执行其他任务。
 * 4、协程依附到目标线程上并会延迟执行
 * 5、每个线程大约会占用1MB的内存空间
 * 6、launch返回Job类型，而async返回Deferred类型。launch与join，async与await
 */
fun main() {
